class Author < ApplicationRecord
  ...
  # scopes
  scope :isap, -> {where("authors.isap = TRUE")}
  scope :not_verified, -> {joins("INNER JOIN article_authors ON article_authors.author_id = authors.id").where("article_authors.status <> 'verified'") }
  ...
  scope :publications_count, -> {isap.joins(:articles).select("authors.id, authors.last_name ||', '|| authors.given_name as full_name, COUNT(*) AS pub_count, authors.orcid").order("pub_count DESC").group("authors.id, authors.given_name, authors.last_name")}
  scope :data_objects_count, -> {isap.joins(:articles).joins("INNER JOIN article_datasets ON article_datasets.article_id = articles.id").where("article_datasets.article_id = articles.id").select("authors.id, authors.last_name ||', '|| authors.given_name as full_name, COUNT(*) AS pub_count").order("pub_count DESC").group("authors.id, authors.given_name, authors.last_name")}
  scope :citations_count, -> {isap.joins(:articles).select("authors.id, authors.last_name ||', '|| authors.given_name as full_name, SUM(articles.referenced_by_count) AS Citations").order("Citations DESC").group("authors.id, authors.given_name, authors.last_name")}
