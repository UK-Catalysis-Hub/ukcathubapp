...
class CrossrefPublication
  ...
  def self.parse_author_affiliations(article_author_id = 212)
    an_art_aut = ArticleAuthor.find(article_author_id)
    affi_parser = AffiliationsParser.new()
    affi_lines = CrAffiliation.where(article_author_id: article_author_id, author_affiliation_id: nil)
    if affi_lines.length > 0
      puts "Afiliatios for Article " + article_author_id.to_s + ": "+ affi_lines.length.to_s
      return_hash = affi_parser.one_by_one_affi(affi_lines)
      puts "\n***********************************************************"
      puts "* Author: " + an_art_aut.id.to_s + " " + an_art_aut.given_name + " " + an_art_aut.last_name   + " Lines: " + affi_lines.count.to_s
      puts "* input lines: "
      affi_parser.print_lines(affi_lines)
      print "\n*RETURN HASH " + return_hash.to_s
      saved_affis = affi_parser.build_and_save_auth_affis(return_hash, an_art_aut.id)
      print "\n*New affilaitions saved: " + saved_affis.to_s
      puts "\n***********************************************************"
    end
  end
  ...
  class AffiliationsParser
    ...
    def initialize()
      ...
      # (need to persist somewhere)
      @country_exceptions = ["Denmark Hill", "UK Catalysis Hub", "Sasol Technology U.K.", "N. Ireland"]
    end # initialize method Affiliations parser
    ...
    def one_by_one_affi(affi_lines, accummulated = [])
      ...
      return affi_hash
    end #one_by_one method AffiliationsParser
    ...
    def split_by_keywords(affi_string)
      ...
      return affiliation_array
    end # split_by_keywords method of AffiliationsParser
    ...
    #########################################################
    # | Get elements from lists of keywords and synonyms  | #
    # V                                                   V #
    #########################################################
    ...
    def get_country_synonym(affi_string)
      ...
      @country_synonyms.keys.each do |ctry_key|
        if cleared_affi_string.include?(ctry_key.to_s)
          all_synonyms.append(ctry_key.to_s)
        end
      end
      return_this = ""
      all_synonyms.each do |a_synonym|
        if a_synonym.length() > return_this.length()
          return_this = a_synonym
        end
      end
      return return_this
    end
